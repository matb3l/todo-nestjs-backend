...
export class ColumnService {
  ...
  async createColumn(
    ...
    const order = (await this.getColumns(user, projectId)).length + 1;
  ...
  async getColumns(user: User, projectId: number): Promise<TaskColumn[]> {
    const project = await this.projectRepository.findOne({
      where: { user: { id: user.id }, id: projectId },
    });
    if (!project) {
      throw new NotFoundException('Проект не найден');
    }
  ...
  async getColumnById(user: User, id: number): Promise<TaskColumn> {
  ...
  async updateColumn(
    ...
    const column = await this.getColumnById(user, id);
    column.title = title;
    return this.columnRepository.save(column);
  }

  async moveColumn(user: User, id: number, order: number): Promise<void> {
    const column = await this.getColumnById(user, id);
    const columns = await this.getColumns(user, column.project.id);
  ...
  async deleteColumn(user: User, id: number): Promise<void> {
    const column = await this.getColumnById(user, id);
    const columns = await this.getColumns(user, column.project.id);
